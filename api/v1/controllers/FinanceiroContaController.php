    ...
    public static function getMapaDeContas(array $data): array
    {
        ...
        try {
            ...
            $parseAnoMes = function($s): array {
                ...
                if      (preg_match('/^(\d{4})[-\/](\d{2})$/', $s, $m)) { $ano = (int)$m[1]; $mes = (int)$m[2]; }
                elseif  (preg_match('/^(\d{2})[-\/](\d{4})$/', $s, $m)) { $mes = (int)$m[1]; $ano = (int)$m[2]; }
                elseif  (preg_match('/^(\d{4})(\d{2})$/',      $s, $m)) { $ano = (int)$m[1]; $mes = (int)$m[2]; }
                else    throw new Exception("ano_mes inválido. Exemplos aceitos: 2025-12, 2025/12, 12/2025, 202512");
            ...
            // Filtro opcional por tipo (D = débito, C = crédito, null = ambos)
            $tipoConta = isset($data['tipo']) ? strtoupper(trim($data['tipo'])) : null;
            if ($tipoConta !== null && !in_array($tipoConta, ['D', 'C'], true)) {
                throw new Exception("tipo inválido. Use 'D' (débito) ou 'C' (crédito).");
            }

            // Coluna de data usada no filtro do período
            $campoData = $tipoData === 'vencimento' ? 'fc.vencimento' : 'fc.emissao';

            // ========= query =========
            $tipoWhere = $tipoConta !== null ? "AND fc.tipo = :tipo" : "";
            ...
            WHERE fc.system_unit_id = :unit
              ...
              $tipoWhere
            ORDER BY $campoData ASC, fc.nome ASC, fc.doc ASC
        ";
            ...
            foreach ($rows as $r) {
                $emissao    = !empty($r['emissao'])    ? date('Y-m-d', strtotime($r['emissao']))    : null;
                $vencimento = !empty($r['vencimento']) ? date('Y-m-d', strtotime($r['vencimento'])) : null;
                $baixaDt    = !empty($r['baixa_dt']) && $r['baixa_dt'] !== '0000-00-00'
                    ? date('Y-m-d', strtotime($r['baixa_dt']))
                    : null;

                $valor   = (float)($r['valor'] ?? 0);
                $dataRef = $tipoData === 'vencimento' ? $vencimento : $emissao;
                ...
                $item = [
                    ...
                    'tipo'             => (string)($r['tipo'] ?? ''),      // D ou C
                    'nome'             => (string)($r['nome'] ?? ''),
                    'entidade'         => (int)($r['entidade'] ?? 0),
                    'cgc'              => (string)($r['cgc'] ?? ''),
                    'doc'              => (string)($r['doc'] ?? ''),
                    'emissao'          => $emissao,
                    'vencimento'       => $vencimento,
                    ...
                    'plano_descricao'  => (string)($r['plano_descricao'] ?? ''),
                    'obs'              => (string)($r['obs'] ?? ''),
                ];
